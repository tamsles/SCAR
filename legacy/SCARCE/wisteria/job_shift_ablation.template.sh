#!/bin/bash
#PJM -N sh___COND___s__SEED__
#PJM -L rscgrp=share
#PJM -L gpu=1
#PJM -L elapse=48:00:00
#PJM --fs /work
#PJM -j

set -Eeuo pipefail

SEED=__SEED__
CONDITION="__COND__"
SHIFT="__SHIFT__"
SOURCE_RATE="__SOURCE_RATE__"
TARGET_RATE="__TARGET_RATE__"
REPO_DIR="__REPO_DIR__"
CONFIG_FILE="${REPO_DIR}/wisteria/config.sh"

if [[ ! -f "${CONFIG_FILE}" ]]; then
    echo "Missing Wisteria configuration: ${CONFIG_FILE}" >&2
    exit 2
fi

# shellcheck source=config.sh
source "${CONFIG_FILE}"
RUN_TESTS=__RUN_TESTS__
cd "${REPO_DIR}"
mkdir -p logs result/adiw_scar/total result/adiw_scar/detail
mkdir -p result/domain_scarce/total result/domain_scarce/detail

JOB_TAG="${PJM_JOBID:-manual_$(date +%Y%m%d_%H%M%S)}"
LOG_FILE="${REPO_DIR}/logs/shift_${CONDITION}_seed${SEED}_${JOB_TAG}.log"
exec > >(tee -a "${LOG_FILE}") 2>&1

echo "Start: $(date --iso-8601=seconds)"
echo "Host: $(hostname)"
echo "Condition: ${CONDITION}"
echo "Shift: ${SHIFT}"
echo "Source SCAR rate: ${SOURCE_RATE}"
echo "Target SCAR rate: ${TARGET_RATE}"
echo "Seed: ${SEED}"

module purge >/dev/null 2>&1 || true
module load aquarius
if [[ -n "${EXTRA_MODULES}" ]]; then
    read -r -a extra_module_array <<< "${EXTRA_MODULES}"
    module load "${extra_module_array[@]}"
fi
if [[ -n "${PYTORCH_MODULE}" ]]; then
    module load "${PYTORCH_MODULE}"
fi
module list

export PYTHONUNBUFFERED=1
export OMP_NUM_THREADS="${OMP_NUM_THREADS:-8}"
export MKL_NUM_THREADS="${MKL_NUM_THREADS:-8}"
export TORCH_HOME="${TORCH_HOME:-${REPO_DIR}/.cache/torch}"
export XDG_CACHE_HOME="${XDG_CACHE_HOME:-${REPO_DIR}/.cache}"
mkdir -p "${TORCH_HOME}" "${XDG_CACHE_HOME}"

if ! command -v "${PYTHON_BIN}" >/dev/null 2>&1; then
    echo "Python interpreter not found: ${PYTHON_BIN}" >&2
    exit 2
fi

"${PYTHON_BIN}" - <<'PY'
import torch
import torchvision

print("PyTorch:", torch.__version__)
print("torchvision:", torchvision.__version__)
print("CUDA available:", torch.cuda.is_available())
if not torch.cuda.is_available():
    raise SystemExit("CUDA is unavailable; stopping before training.")
print("GPU:", torch.cuda.get_device_name(0))
PY

if [[ "${RUN_TESTS}" == "1" ]]; then
    "${PYTHON_BIN}" -m unittest discover -s tests -v
fi

COMMON_ARGS=(
    -ds mnist
    -mo mlp
    -shift "${SHIFT}"
    -target_rotation 30
    -label_shift 1
    -target_weak_size 1000
    -source_size 0
    -source_gen independent
    -target_gen independent
    -source_scar_rate "${SOURCE_RATE}"
    -target_scar_rate "${TARGET_RATE}"
    -risk_correction abs
    -bs 256
    -op adam
    -lr 1e-3
    -wd 1e-5
    -ep 200
    -run_times 1
    -seed "${SEED}"
    -gpu 0
    -workers 0
)

echo "Starting ADIW-SCAR for ${CONDITION}, seed ${SEED}"
"${PYTHON_BIN}" main_adiw_scar.py \
    "${COMMON_ARGS[@]}" \
    -target_bs 256 \
    -target_prior uniform \
    -eta -1 \
    -adiw_steps 1 \
    -adiw_lr 1.0 \
    -adiw_max_weight 50 \
    -adiw_kernel_quantile 0.5

echo "Starting pooled SCAR without ADIW for ${CONDITION}, seed ${SEED}"
"${PYTHON_BIN}" main_domain_scarce.py \
    "${COMMON_ARGS[@]}" \
    -domain pooled \
    -class_prior uniform

echo "Finish: $(date --iso-8601=seconds)"
echo "Log: ${LOG_FILE}"
