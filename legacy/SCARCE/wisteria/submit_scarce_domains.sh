...
for seed in "${seeds[@]}"; do
    ...
    job_file="${SCRIPT_DIR}/generated/job_scarce_domains_seed_${seed}.sh"
    sed \
        -e "s/__SEED__/${seed}/g" \
        -e "s/__REPO_DIR__/${escaped_repo_dir}/g" \
        -e "s/__JOB_NAME__/scarce_dom/g" \
    ...
    echo "Submitting train/test-domain SCARCE seed ${seed}: ${job_file}"
