"""Run one B4 class-heterogeneous MNIST task."""
...
def expand_tasks(
    ...
    return [
        {
            "setting": setting,
            "class_rotations": rotations,
            "method": method,
            "seed": int(seed),
        }
        for (setting, rotations), method, seed in itertools.product(
            config["settings"].items(), config["methods"], seeds
        )
    ]
...
def task_arguments(
    ...
    return [
        ...
        task["setting"],
        "--class-rotations",
        ",".join(str(value) for value in task["class_rotations"]),
        "--target-weak-size",
...
def parse_args(
    ...
    parser.add_argument(
        "--config",
        default="configs/phase_b/b4_heterogeneous_mnist/config.json",
    )
    parser.add_argument("--phase", default="B4")
    parser.add_argument(
        "--experiment-mode",
    ...
    parser.add_argument("--results-root", default="outputs/phase_b/runs/B4")
    parser.add_argument(
